...
public class IntentMetrics {
    ...
    private final Map<IntentType, LongAdder> intentCounts = new EnumMap<>(IntentType.class);
    ...
    public IntentMetrics() {
        ...
        for (IntentType type : IntentType.values()) {
            intentCounts.put(type, new LongAdder());
        }
    }
    ...
    public synchronized void record(IntentResult result, long latencyMs) {
        ...
        intentCounts.get(result.getIntent()).increment();
    ...
    public synchronized Map<String, Object> snapshot() {
        ...
        Map<String, Long> intentHits = new LinkedHashMap<>();
        for (Map.Entry<IntentType, LongAdder> entry : intentCounts.entrySet()) {
            intentHits.put(entry.getKey().name(), entry.getValue().sum());
        }
        snapshot.put("intentCounts", intentHits);
        ...
        snapshot.put("p95LatencyMs", percentile(0.95));  // 别名，匹配 spec 命名
